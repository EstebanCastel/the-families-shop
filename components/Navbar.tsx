...
export default function Navbar() {
  ...
  return (
    <>
      <div className="bg-[var(--ink)] py-2 text-center text-[11px] tracking-[0.14em] text-white upper">
        ...
        <nav className="mx-auto flex h-14 max-w-[1400px] items-center justify-between px-4 md:h-16 md:px-8">
          ...
          <div className="flex items-center gap-5">
            ...
            <button onClick={() => setOpen(true)} className="tnav flex items-center gap-1.5" aria-label="Carrito">
              Cesta
              <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--ink)] px-1 text-[10px] text-white">{count}</span>
            </button>
          </div>
        </nav>
      </header>
