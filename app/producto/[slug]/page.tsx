...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-0 md:px-8 md:py-6">
      <nav className="px-4 py-3 text-[12px] text-[var(--muted)] md:px-0">
        <Link href="/" className="hover:text-[var(--ink)]">Inicio</Link> / <Link href={`/categoria/${p.category}`} className="hover:text-[var(--ink)]">{categoryLabel(p.category)}</Link> / <span className="text-[var(--ink)]">{p.name}</span>
      </nav>
      ...
      <div className="md:grid md:grid-cols-[1.6fr_1fr] md:gap-10">
        ...
        <div className="px-4 pb-10 pt-6 md:px-0 md:pt-2">
          <div className="md:sticky md:top-24">
            <p className="tnav mb-2 text-[var(--muted)]">{categoryLabel(p.category)}</p>
            <h1 className="text-xl font-medium md:text-2xl">{p.name}</h1>
            <p className="mt-2 text-lg">{formatCOP(p.price)}</p>
            <p className="mt-1 text-[13px] text-[var(--muted)]">Color: {p.color}</p>

            <div className="mt-7">
              <AddToCart product={p} />
            </div>

            <p className="mt-5 text-[12px] text-[var(--muted)]">Envío gratis desde $300.000 · Devoluciones en 30 días · Hecho a pedido en 2–4 semanas</p>
            ...
            <div className="mt-8 border-t border-[var(--line)]">
              ...
              <details className="border-b border-[var(--line)]">
                ...
                <ul className="pb-5 text-[13px] leading-relaxed text-[#444]">
                  ...
                  <li className="mt-2 text-[var(--muted)]">Lavar a mano en frío. Secar en plano.</li>
                </ul>
              </details>
              <details className="border-b border-[var(--line)]">
